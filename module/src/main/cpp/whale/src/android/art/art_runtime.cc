...
#define CHECK_FIELD(field, value)  \
    ...
    if (api_level_ >= ANDROID_S)
        expected_access_flags |= kAccObsoleteObject;
    hookEncodeArtMethod();
    ...
    if (api_level_ < ANDROID_S) {
        ...
        method_offset_.dex_code_item_offset_offset_ = -1;
        method_offset_.dex_method_index_offset_ = access_flags_offset + sizeof(u4);
        method_offset_.method_index_offset_ = access_flags_offset + sizeof(u4) ;
    }
...
ArtRuntime::HookMethod(JNIEnv *env, jclass decl_class, jobject hooked_java_method,
    ...
    param->origin_code_item_off = (api_level_ >= ANDROID_S) ? 0 : hooked_method.GetDexCodeItemOffset();
...
ArtRuntime::InvokeOriginalMethod(jlong slot, jobject this_object, jobjectArray args) {
    ...
    if (param->decl_class_ != decl_class) {
        ...
        if (param->decl_class_ != decl_class) {
            ...
            if(param->origin_code_item_off != 0){
